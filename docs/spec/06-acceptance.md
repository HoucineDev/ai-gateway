...
Not asserted by this proposal. The load test harness (`scripts/loadtest.py` over `aigw.testing.loadtest`) reports RPS, concurrent streams, payload sizes (`--prompt-words`, `--max-tokens`), the stream/unary/embeddings mix, TTFB and total latency p50/p95/p99, error codes, cache outcomes and deployment spread, and — with `--upstream` — the p50/p95/p99 **added** gateway latency (gateway TTFB minus upstream TTFB, per percentile) against the mock upstream so the gateway's own overhead is measured in isolation. `--max-error-rate`, `--max-p95-ms`, `--min-rps` and `--max-added-p95-ms` turn a run into a CI gate (exit 1). `tests/test_load.py` drives the same harness in-process and asserts the ledger invariants after concurrent mixed traffic (no reservation left, budget spend = settled usage, one attempt per request). Targets are set with the workload inputs from 00-overview § 5.
